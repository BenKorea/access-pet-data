---
title: "Access PET DATA"
author: 김병일
description: "Access 구현 핵의학과 PET 판독/준비기록"
date: "2025-07-23"
...
#| fig-width: 10
#| fig-height: 6
#| fig-cap: "Fig 1 OCS PET 판독건수"

log_info("OCS 총판독건수: {nrow(readings_ocs)}건")
...
log_debug("OCS 판독 중 ExamDate NA: {nrow(readings_ocs_na)}건")
...
log_info("readings_ocs 기간을 readings_pacs과 같도록 2003-05-12 ~ 2016-12-31으로 진행합니다.")
...
log_debug("OCS 데이터 (2003-05-12~2016-12-31): {n_readings_ocs_for_matching}건")
...
log_debug("중복된 readings_ocs: {nrow(duplicated_readings_ocs_for_matching)}건")
ocs_unique_keys <- unique(readings_ocs_for_matching[, .(PtID, ExamDate)])
n_ocs_unique <- nrow(ocs_unique_keys)
log_debug("OCS unique keys: {n_ocs_unique}")
...
log_debug("PACS unique keys: {n_pacs_unique}")
...
```{r}
#| label: all_readings_without_preparation
#| echo: false
#| message: false
#| warning: false
#| fig-width: 10
...
# 1. 판독/검사기록 유니크 리스트 생성
readings_access_keys <- unique(readings_access[, .(PtID, ExamDate)])
readings_ocs_keys    <- unique(readings_ocs[, .(PtID, ExamDate)])
readings_pacs_keys   <- unique(readings_pacs[, .(PtID, ExamDate)])
all_readings_keys <- unique(rbindlist(list(readings_access_keys, readings_ocs_keys, readings_pacs_keys)))

# 2. 준비기록 유니크 리스트 생성
preparations_access_keys <- unique(preparations_access[, .(PtID, ExamDate)])
preparations_ocs_keys    <- unique(preparations_ocs[, .(PtID, ExamDate)])
all_preparations_keys <- unique(rbindlist(list(preparations_access_keys, preparations_ocs_keys)))

# 3. 판독은 있으나 준비기록이 없는 리스트 추출
all_readings_without_preparation <- all_readings_keys[!all_preparations_keys, on = .(PtID, ExamDate)]

# 결과 확인
log_info("준비기록 없는 판독/검사기록 건수: {nrow(all_readings_without_preparation)}")
# head(all_readings_without_preparation)

# 월별 시각화
my_plot_monthly_counts(all_readings_without_preparation, ExamDate, "준비기록 없는 판독/검사기록")
```

## 2009년~2010년 판독/검사기록

```{r}
#| label: analyze_2009_2010_no_preparation
#| echo: false
#| message: false
#| warning: false
#| fig-width: 10
...
# 2009년 데이터만 추출
peak_2009_2010 <- all_readings_without_preparation[
  ExamDate >= as.Date("2009-01-01") & ExamDate <= as.Date("2010-12-31")
]

# 월별 시각화만 출력
my_plot_monthly_counts(peak_2009_2010, ExamDate, "2009~2010년 준비기록 없는 판독/검사기록")
```

## 2009년  7월~2010년2월 날짜별 건수 시각화

```{r}
#| label: plot_daily_2009_07_to_2010_02_no_preparation
#| echo: false
#| message: false
#| warning: false
#| fig-width: 12
...
# 2009년 7월~2010년 2월 기간 추출
daily_peak <- all_readings_without_preparation[
  ExamDate >= as.Date("2009-07-01") & ExamDate <= as.Date("2010-02-28")
]

# 날짜별 건수 집계
library(ggplot2)
dt_daily <- daily_peak[, .N, by = ExamDate]

ggplot(dt_daily, aes(x = ExamDate, y = N)) +
  geom_col(fill = "steelblue") +
  labs(title = "2009년 8월~2010년 1월 준비기록 없는 판독/검사기록 (날짜별)",
       x = "ExamDate", y = "건수") +
  theme_minimal() +
